...
function TestimonialCard({
  ...
  return (
    <a
      ...
      className={cn(
        "relative flex h-auto min-h-[140px] sm:min-h-[180px] w-[260px] sm:w-[380px] -skew-y-[8deg] select-none flex-col rounded-2xl border border-zinc-800 bg-zinc-950 backdrop-blur-sm px-3 sm:px-4 py-3 sm:py-4 transition-all duration-500 hover:border-zinc-700 hover:bg-zinc-900 cursor-pointer",
      ...
      <div className="flex items-start gap-2 sm:gap-3 mb-2 sm:mb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            <span className="font-bold text-zinc-50 truncate text-xs sm:text-base">{username}</span>
            {verified && <VerifiedBadge />}
          </div>
          <span className="text-zinc-500 text-[10px] sm:text-sm">{handle}</span>
        </div>
        <TwitterIcon className="size-4 sm:size-5 text-zinc-50 shrink-0" />
      </div>

      {/* Content */}
      <p className="text-zinc-300 text-xs sm:text-[15px] leading-relaxed mb-2 sm:mb-3 line-clamp-3 sm:line-clamp-4">
        {content}
      </p>

      {/* Footer */}
      <div className="flex items-center justify-between text-zinc-500 text-[10px] sm:text-sm mt-auto">
        <span>{date}</span>
...
export default function Testimonials({ cards }: TestimonialsProps) {
  ...
  const defaultCards: TestimonialCardProps[] = [
    {
      className:
        "[grid-area:stack] hover:-translate-y-10 before:absolute before:w-[100%] before:outline-1 before:rounded-2xl before:outline-border before:h-[100%] before:content-[''] before:bg-blend-overlay before:bg-zinc-950/60 grayscale-[100%] hover:before:opacity-0 before:transition-opacity before:duration-500 hover:grayscale-0 before:left-0 before:top-0",
      avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=JC",
      username: "Johan Jovin Cheeran",
      handle: "@sarahchen",
      content: "Silencly completely replaced my scratchpad. I can blurt out highly unorganized thoughts about code design, and it formats them into clear JIRA tasks instantly. It's easily 10x faster.",
      date: "Jan 3, 2026",
      verified: true,
      likes: 42,
      retweets: 8,
    ...
    },
    {
      ...
      handle: "@mikej_dev",
      content: "I use Silencly to dictate UX feedback while reviewing mockups. What used to take 20 minutes of tedious keyboard typing now takes a 2-minute voice braindump.",
      date: "Jan 2, 2026",
      verified: true,
      likes: 28,
      retweets: 5,
    ...
    },
    {
      ...
      username: "Anubhav Sapkota",
      handle: "@alexrivera",
      content: "The custom vocabulary is a game changer. It actually spells our company acronyms and specific API endpoints correctly. I couldn't go back to standard voice tools.",
      date: "Jan 1, 2026",
      verified: true,
      likes: 156,
      retweets: 23,
